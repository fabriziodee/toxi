...
let handler = async (m, { conn, text }) => {
  ...
  if (!code) throw '*Invaild Link, Please Enter a Vaild Link*'
...
${res.id}
*Title:* ${res.subject}
*Created* by @${res.id.split('-')[0]} pada *${formatDate(res.creation * 1000)}*${res.subjectOwner ? `
*Title changed* by @${res.subjectOwner.split`@`[0]} pada *${formatDate(res.subjectTime * 1000)}*`: ''}${res.descOwner ? `
*Description Changed* by @${res.descOwner.split`@`[0]} pada *${formatDate(res.descTime * 1000)}*` : ''}
